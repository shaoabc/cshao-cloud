...
public class ManageAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        if (this.isProtectedRequest(request)) {
            ...
            if (jwtTokenUtil.isTokenExpired(authToken, ClientOriginEnum.SLL_PLAT.getSecret())) {
                throw new FailTokenException("令牌已过期，请重新登录");
            } else {
                final AuthUser platUser = jwtTokenUtil.toAuthUser(authToken, ClientOriginEnum.SLL_PLAT.getSecret());
                final Collection<? extends GrantedAuthority> authorities = jwtTokenUtil.getAuthorities(authToken, ClientOriginEnum.SLL_PLAT.getSecret());
    ...
    private Boolean isProtectedRequest(HttpServletRequest httpServletRequest) {
        return PATHMATCHER.match("/api/manage/**", httpServletRequest.getServletPath())
